...
from Caulimate.Utils.Tools import check_tensor, check_array, load_yaml, makedir, lin_reg_init, dict_to_class, save_log, bin_mat, center_and_norm
...
if __name__ == '__main__':
    ...
    dataset = CESM2_grouped_dataset(args.data_path, num_area=1)[0]
    ...
    B_init, reg_mask = lin_reg_init(center_and_norm(dataset.X), thres=args.reg_thres)
    B_init = check_tensor(B_init, dtype=torch.float32)
    reg_mask = bin_mat(B_init)
    print('--- Mask by regression: \n {} \n------ num: {} \n------ ratio: {}'.format(reg_mask, int(reg_mask.sum()), reg_mask.sum() / args.d_X ** 2))

    mask = eud_mask * reg_mask
    print('--- Mask in training: \n {} \n------ num: {} \n------ ratio: {}'.format(mask, int(mask.sum()), mask.sum() / args.d_X ** 2))
    ...
    model = GolemModel(args, args.d_X, dataset.coords, mask=mask, in_dim=1, equal_variances=True, seed=1,)
